...
var _ = Describe("remediationv1alpha1.NodeHealthCheck Validation", func() {
	...
	Context("Creating or updating a NHC", func() {
		...
		Context("with negative minHealthy", func() {
			...
			It("should be denied", func() {
				Expect(validator.validate(context.Background(), nhc)).To(MatchError(ContainSubstring("minHealthy must not be negative")))
			})
		})
		...
		Context("with negative maxUnhealthy", func() {
			...
			It("should be denied", func() {
				Expect(validator.validate(context.Background(), nhc)).To(MatchError(ContainSubstring("maxUnhealthy must not be negative")))
			})
		})
		...
		Context("with maxUnhealthy and MachineDeletionRemediationTemplate as escalating remediation", func() {
			BeforeEach(func() {
				...
				nhc.Spec.MinHealthy = nil
				nhc.Spec.MaxUnhealthy = &mh
				nhc.Spec.EscalatingRemediations = []remediationv1alpha1.EscalatingRemediation{{
	...
	Context("During ongoing remediation", func() {
		...
		var nhcOld *remediationv1alpha1.NodeHealthCheck
		var nhcNew *remediationv1alpha1.NodeHealthCheck

		BeforeEach(func() {
			mh := intstr.FromString("51%")
			nhcOld = &remediationv1alpha1.NodeHealthCheck{
				ObjectMeta: metav1.ObjectMeta{
					Name: "test",
				},
				Spec: remediationv1alpha1.NodeHealthCheckSpec{
	...
	Context("Test isRemediating", func() {
		...
		When("unhealthy node isn't remediated yet", func() {
			BeforeEach(func() {
				nhc.Status.UnhealthyNodes = []*remediationv1alpha1.UnhealthyNode{
					{
						Name:                       "test",
						Remediations:               nil,
						ConditionsHealthyTimestamp: nil,
			...
			It("should return false", func() {
				Expect(isRemediating(nhc)).To(BeFalse())
			})
		})
		...
		When("unhealthy node is remediated", func() {
			...
			It("should return true", func() {
				Expect(isRemediating(nhc)).To(BeTrue())
			})
		})
	})
})

func setEscalatingRemediations(nhc *remediationv1alpha1.NodeHealthCheck) {
	nhc.Spec.RemediationTemplate = nil
	nhc.Spec.EscalatingRemediations = []remediationv1alpha1.EscalatingRemediation{
		{
			RemediationTemplate: v1.ObjectReference{
